...
const PomodoroPage: React.FC = () => {
  ...
  const handleTimerComplete = useCallback(async () => {
    ...
    if (sessionType === SessionType.FOCUS) {
      ...
      setCycles(newCycles);
      setSessionType(
        newCycles % 4 === 0 ? SessionType.LONG_BREAK : SessionType.SHORT_BREAK
      );
    } else {
    ...
    if (taskId && !isActive) {
      ...
      if (taskTitle) setCurrentTask(taskTitle);
        ...
        TaskService.getTask(taskId).then(
          (res) => res.data.success && setCurrentTask(res.data.data.title)
        );
    }
  }, [location.search, isActive]);

  useEffect(() => {
    // Effect to set timer value
    if (isActive || timerCompleted) return;
    let newMinutes = 25;
    switch (sessionType) {
      case SessionType.FOCUS:
        newMinutes = customTime;
        break;
    ...
    document.title = `${formatTimeLocal(
      newMinutes,
      0
    )} - ${sessionType} - FocusBuddy`;
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const elapsedSeconds = Math.floor(
        (now - (startTime ?? now) - totalPausedTime) / 1000
      );
  ...
  useEffect(() => {
    ...
    fetchAvailableTasks();
    fetchDailyStats();
    audioRef.current = new Audio("/sounds/achievement-bell.mp3"); // Ensure this path is correct
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      if (isActive && !isPaused) {
        e.preventDefault();
        e.returnValue = "Timer is active. Sure you want to leave?";
      }
    };
    ...
    isActive,
    isPaused,
  ]); // Added isActive/isPaused dependencies

  useEffect(() => {
    // Effect to close task dropdown on outside click
    const handleClickOutside = (event: MouseEvent) => {
      if (
        taskDropdownRef.current &&
        !taskDropdownRef.current.contains(event.target as Node) &&
        taskInputRef.current &&
        !taskInputRef.current.contains(event.target as Node)
      ) {
  ...
  const handlePauseTimer = () => {
    ...
    if (animationFrameRef.current)
      cancelAnimationFrame(animationFrameRef.current);
    document.title = `Paused - ${formatTimeLocal(
      timer.minutes,
      timer.seconds
    )} - FocusBuddy`;
  };
  ...
  const filteredTasksForDropdown = availableTasks.filter(
    (task) =>
    ...
    <div className={styles.pomodoroPage}>
      ...
      <div className={styles.timerContainer}>
        ...
        {sessionType === SessionType.FOCUS && (
          ...
          <div ref={taskDropdownRef} className={styles.taskInputWrapper}>
            {" "}
      ...
      <div className={styles.streakGraphContainer}>
        ...
        {loadingStats ? (
          <div className={styles.loadingMessage}>Loading streak data...</div>
        ) : (
          <DailyStreakGraph dailyData={dailyStats} />
        )}
      </div>
